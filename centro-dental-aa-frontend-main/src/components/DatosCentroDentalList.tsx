...
const DatosCentroDentalList: React.FC = () => {
    ...
    const manualSections: ManualSection[] = [
        {
            title: 'Gestión de Datos del Centro Dental',
            content: 'Aquí puede administrar la información del Centro Dental (ej. Nombre, Dirección, Teléfonos, QR). Use el botón "+ Nuevo Dato" para agregar configuración, aunque se recomienda tener solo uno activo.'
        },
        {
            title: 'Dar de Baja y Reactivar',
            content: 'Para registros activos, el botón rojo cambia el estado a "Inactivo". Para registros inactivos, aparece un botón verde que permite reactivarlos.'
        }];
    ...
    const fetchDatos = async () => {
        try {
            // we have a REST endpoint for datos-centro-dental 
            // wait, our backend endpoint does not support pagination natively in the service I generated (it just uses find()).
            // But I'll simulate pagination on frontend or just use it as is.
            const response = await api.get<DatosCentroDental[]>(`/datos-centro-dental`);
            
            // We need to filter manually if search is present since backend doesn't implement it
            ...
            alert('Error al cargar los datos del centro dental');
        }
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        if (result.isConfirmed) {
            try {
                await api.put(`/datos-centro-dental/${id}`, { estado: 'inactivo' });
    ...
    const handleReactivate = async (id: number) => {
        ...
        if (result.isConfirmed) {
            try {
                await api.put(`/datos-centro-dental/${id}`, { estado: 'activo' });
    ...
    const handlePrint = async () => {
        try {
            const response = await api.get<DatosCentroDental[]>(`/datos-centro-dental`);
            let allDatos = response.data;
            if (searchTerm) {
                const lowerSearch = searchTerm.toLowerCase();
                allDatos = allDatos.filter(d => 
                    d.nombre?.toLowerCase().includes(lowerSearch) || 
                    d.direccion?.toLowerCase().includes(lowerSearch)
                );
            }
            ...
            const printContent = `
                ...
                <html>
                <head>
                    <title>Datos del Centro Dental</title>
                    <style>
                ...
                <body>
                    ...
                    <table>
                        ...
                        <tbody>
                            ${allDatos.map((e, index) => `
                                <tr>
                                    <td>${index + 1}</td>
                                    <td>${e.nombre || '-'}</td>
                                    <td>${e.telefono || '-'} / ${e.celular || '-'}</td>
        ...
        <div className="content-card p-6 bg-gray-50 dark:bg-gray-800 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 no-print gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Configuración general de la clínica</p>
                    </div>
                </div>
                <div className="flex gap-2 flex-wrap justify-center md:justify-end items-center">
                    ...
                    >
                        <span className="text-xl font-bold">+</span> Nuevo Registro
                    </button>
                </div>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Teléfonos</th>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {datos.map((e, index) => (
                            <tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">{e.nombre}</td>
                                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-200">{e.direccion}</td>
                                <td className="px-6 py-4 text-sm text-gray-900 dark:text-gray-200">
                                    {e.telefono && <div>T: {e.telefono}</div>}
                                    {e.celular && <div>C: {e.celular}</div>}
                                </td>
            ...
            {datos.length === 0 && (
                <p className="text-center mt-5 text-gray-500 dark:text-gray-400">
                    {searchTerm ? 'No se encontraron resultados' : 'No hay registros del Centro Dental'}
                </p>
            )}
            ...
            <ManualModal
                ...
                title="Manual de Usuario - Datos Centro Dental"
