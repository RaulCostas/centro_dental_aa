...
const ArancelList: React.FC = () => {
    ...
    const exportToExcel = async () => {
        try {
            ...
            const excelData = allAranceles.map((a: any) => {
                const row: any = {
                    'ID': a.id,
    ...
    const exportToPDF = async () => {
        try {
            ...
            // Add logo
            const logoPath = '/logo-clinica-dental.jpg';
            try {
                const img = new Image();
                img.src = logoPath;
                ...
                if (img.complete && img.naturalHeight !== 0) {
                    // [Antigravity: removed logo from print] doc.addImage(img, 'PNG', 14, 10, 30, 15);
                }
            } catch (error) {
            ...
            const tableData = allAranceles.map((a: any, index: number) => {
                const row = [
                    index + 1,
                    a.detalle,
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead className="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-300 uppercase tracking-wider">Odontograma</th>
                    ...
                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        {aranceles.map((a, index) => (
                            <tr key={a.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all">
                                ...
                                <td className="p-3 text-gray-800 dark:text-gray-300">
                                    {a.odontogramaColor || a.odontogramaFigura ? (
                                        <div className="flex items-center gap-2">
                                            {a.odontogramaColor && (
                                                <span
                                                    className="w-4 h-4 rounded-full border border-gray-350 dark:border-gray-600 block shadow-sm"
                                                    style={{ backgroundColor: a.odontogramaColor }}
                                                    title={a.odontogramaColor}
                                                />
                                            )}
                                            <span className="text-xs uppercase font-bold text-gray-600 dark:text-gray-400">
                                                {a.odontogramaFigura ? a.odontogramaFigura.replace('_', ' ') : '-'}
                                            </span>
                                        </div>
                                    ) : (
