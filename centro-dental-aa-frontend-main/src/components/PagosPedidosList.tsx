...
const PagosPedidosList: React.FC = () => {
    ...
    const handlePrintRecibo = async (pago: PagosPedidos) => {
        try {
            ...
            doc.text(`Comprobante generado automáticamente por el sistema CENTRO DENTAL A&A`, 105, 290, { align: 'center' });

            // Auto print and open
            const receiptLogo = '/logo-clinica-dental.jpg';
            if (receiptLogo) {
                try {
                    const logo = await new Promise<HTMLImageElement>((resolve, reject) => {
                        const img = new Image();
                    ...
                    // [Antigravity: removed logo from print] doc.addImage(logo, 'PNG', 15, 15, 25, 10);
                    // [Antigravity: removed logo from print] doc.addImage(logo, 'PNG', 15, 163, 25, 10);
                } catch (error) {
