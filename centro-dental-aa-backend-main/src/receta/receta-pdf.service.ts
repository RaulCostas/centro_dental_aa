...
export class RecetaPdfService {
    ...
    async generateRecetaPdf(receta: any): Promise<Buffer> {
        ...
        try {
            ...
            console.error('Error fetching datos_centro_dental in PDF generation:', error);
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            if (receta.diagnostico) {
                content.push({
                    text: [
                        { text: 'DIAGNÓSTICO: ', bold: true, color: '#2c3e50', fontSize: 10 },
                        { text: receta.diagnostico, color: '#333333', fontSize: 10 }
                    ],
                    margin: [0, 0, 0, 15]
                });
            }
            ...
            const docDefinition = {
                ...
                footer: (currentPage: number, pageCount: number) => {
                    ...
                    if (currentPage === pageCount) {
                        if (doctorSignature) {
                            footerStack.push({
                                stack: [
                                    ...
                                    {
                                        text: 'Msc. Dr. Alfredo Dimitri Antequera Villagra\nCirujano Dentista\nM.P. No. 317 Col. 996',
                                        fontSize: 7,
                                        bold: true,
                                        alignment: 'center',
                                        margin: [0, 0, 0, 2]
                                    ...
                                    {
                                        text: 'Máster en Implantología Oral\nEndodoncia',
                                        fontSize: 6,
                                        color: '#555555',
                                        alignment: 'center',
                                        margin: [0, 0, 0, 0]
                        ...
                        // Add footer row from centroDental
                        if (centroDental) {
                    ...
                    footerStack.push({
                        columns: [
                            {
                                text: 'El presente documento es una receta médica válida emitida por CENTRO DENTAL A&A.',
                                fontSize: 8,
                                color: '#333333',
                                alignment: 'left',
                                width: '*'
